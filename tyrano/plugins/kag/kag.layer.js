tyrano.plugin.kag.layer = {
  ...
  init: function () {
    ...
    this.appendLayer(layer_obj_click, 'root_layer_system')
    $('body').append(
      // ゲーム画面の外側にもクリック判定を持たせたいので、cloneしてbodyに直接append
      layer_obj_click.clone(true).css('z-index', 1),
      // 所定の場面で↑のレイヤーをクリックさせたくないため、ガード用のdivを追加
      $('<div>').addClass('layer_event_guard').css('display', 'none')
    )

    this.layer_event = $('.layer_event_click')
    var layer_menu = $(
  ...
  setLayerHtml: function (layer) {
    var that = this
    const fixLayer = this.kag.layer.getLayer('fix')
    ...
    for (key in layer.layer_fix) fixLayer.append($(layer.layer_fix[key]))
    $('.blendlayer').remove()
    for (key in layer.layer_blend) {
      var obj = $(layer.layer_blend[key])
      obj.css('background-image') && obj.css('background-color', '')
