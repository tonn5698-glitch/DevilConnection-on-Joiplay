tyrano.plugin.kag.tag.playbgm = {
  ...
  play: function (pm) {
    ...
    if ('bgm' === target) {
      ...
      that.kag.stat.current_bgm_loop = pm.loop
      ...
      that.kag.stat.current_se_vol || (that.kag.stat.current_se_vol = {})
      that.kag.stat.current_se_vol[pm.buf] = pm.volume
    }
...
tyrano.plugin.kag.tag.stopbgm = {
  ...
  start: function (pm) {
    ...
    for (key in target_map)
      ('true' !== pm.buf_all && key != pm.buf) ||
        (function () {
          ...
          if ('true' == pm.fadeout && _audio_obj.volume() > 0) {
    ...
    // stopbgm した時に lbgmstop もするようにする
    const stopTag = pm.target === 'se' ? 'lsestop' : 'lbgmstop'
    that.kag.ftag.startTag(stopTag, {
      stop: 'true',
      buf: pm.buf,
      time: pm.time,
    })
...
tyrano.plugin.kag.tag.bgmopt = {
  ...
  start: function (pm) {
    ...
    if ('true' == pm.effect && 0 == this.kag.define.FLAG_APRI) {
      var new_volume =
        parseFloat(parseInt(pm.volume) / 100) *
        parseFloat(parseInt(this.kag.stat.current_bgm_vol) / 100)
      if ('' == pm.buf)
...
tyrano.plugin.kag.tag.seopt = {
  ...
  start: function (pm) {
    ...
    if ('true' == pm.effect && 0 == this.kag.define.FLAG_APRI) {
      var new_volume =
        parseFloat(parseInt(pm.volume) / 100) *
        parseFloat(parseInt(this.kag.stat.current_se_vol[pm.buf]) / 100)
      if ('' == pm.buf)
...
tyrano.plugin.kag.tag.speak_on = {
  ...
  start: function (pm) {
    'speechSynthesis' in window
      ...
      : console.error(
          '*error: Môi trường này không hỗ trợ chức năng đọc văn bản [speak_on]'
        )
    this.kag.ftag.nextOrder()
  },
}
