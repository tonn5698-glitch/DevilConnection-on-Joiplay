tyrano.plugin.kag = {
  ...
  stat: {
    ...
    sysview: {
      ...
      saveList: './tyrano/html/save_list.html',
    },
    chara_pos_mode: 'true',
    chara_effect: 'swing',
    chara_ptext: '',
    ...
    if (fs.existsSync(patch_path)) {
      ...
      if ('asar' != unzip_path) {
        ...
        if ('darwin' == process.platform) {
          alert(
            'Vui lòng chọn file thực thi game (.app) để áp dụng bản vá.'
          )
        ...
        ;(async () => {
          ...
          $.alert(
            'Đã áp dụng bản vá. Vui lòng khởi động lại.',
            function () {
  ...
  init_game: function () {
    ...
    if ($.isElectron() && 'file' == that.kag.config.configSave) {
      ...
      // Get save key
      const saveKey = window.api
        ? window.api.getSaveKey()
        : localStorage.getItem(that.save_key_id)
      if (saveKey) {
        // save key exists
        that.save_key_val = saveKey
      } else if (!window.api || saveKey == null) {
        // save key does not exists & standalone
        ...
        // WTF are you doing
        alert($.lang('steam_login_failed'))
        return !1
      }
      var tmp_array = that.menu.getSaveData()
      if (!saveKey && tmp_array.hash != that.save_key_val) {
    ...
    var j_message_inner = $(
      "<div class='message_inner' style='z-index:2001'></div>"
    )
    ...
    that.kag.tmp.message_user_scrolled = false
    j_message_inner[0].addEventListener('scroll', function() {
      var el = this
      var atBottom = el.scrollHeight - el.scrollTop - el.clientHeight < 5
      that.kag.tmp.message_user_scrolled = !atBottom
    })
    for (
    ...
    if (this.kag.config.vchat && 'true' == this.kag.config.vchat) {
      ...
      $('#vchat_base').on('click', e => {
        $('.layer_event_click').first().trigger('click')
        e.preventDefault()
      })
    }
      ...
      setTimeout(function () {
        !(function () {
          var player_back_cnt = 0,
            j_menu_button = $(
              "<div id='player_menu_button' class='player_menu_area' style='display:none;opacity:0.6;border-radius:5px;padding:10px;margin:10px;cursor:pointer;position:absolute;left:0px;top:0px;background-color:white;font-size:2em'><span style='color:#6495ED'>Menu</span></div>"
            ),
            j_menu_area = $(
              "<div style='display:none;position:absolute;left:10px;top:10px;font-size:2em'></div>"
            ),
            j_end_button = $(
              "<div class='player_menu_area' id='player_end_button' style='opacity:0.6;border-radius:5px;padding:10px;margin:10px 10px 10px 10px;cursor:pointer;left:0px;top:0px;background-color:white;'><span style='color:#6495ED'>Về trang chủ</span></div>"
            ),
            j_auto_button = $(
              "<div class='player_menu_area' id='player_auto_button' style='opacity:0.6;border-radius:5px;padding:10px;margin:10px 10px 10px 10px;cursor:pointer;left:0px;top:0px;background-color:white;'><span style='color:#6495ED'>Tự động</span></div>"
            ),
            j_skip_button = $(
              "<div class='player_menu_area' id='player_skip_button' style='opacity:0.6;border-radius:5px;padding:10px;margin:10px 10px 10px 10px;cursor:pointer;left:0px;top:0px;background-color:white;'><span style='color:#6495ED'>Bỏ qua</span></div>"
            )
    ...
    'appJsInterface' in window
      ...
      : $.confirm(
          ...
          function () {
            ...
            layer_menu.html('')
            layer_menu.hide()
            that.kag.ftag.startTag('jump', { storage: 'go_to_title.ks' })
          },
          function () {
            return !1
          }
  ...
  appendCharToMessage: function (jtext, c) {
    jtext.find('p').find('.current_span').append(c)
  },
