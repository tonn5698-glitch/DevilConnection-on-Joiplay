...
tyrano.plugin.kag.tag.if = {
  ...
  start: function (pm) {
    if (this.kag.embScript(pm.exp)) {
      ...
      i > 1900 && this.kag.error('Lỗi trong câu lệnh If')
    }
  },
}
...
tyrano.plugin.kag.tag.call = {
  ...
  start: function (pm) {
    ...
    back_pm.line = this.kag.stat.current_line
...
tyrano.plugin.kag.tag.macro = {
  ...
  start: function (pm) {
    ...
    i > 1900 && this.kag.error('Macro chưa được đóng')
  },
}
...
tyrano.plugin.kag.tag.autosave = {
  ...
  start: function (pm) {
    ...
    this.kag.menu.snapSave(
      pm.title,
      function () {
        that.kag.menu.doSetAutoSave(pm.title)
        that.kag.ftag.nextOrder()
      },
      'false'
    )
...
tyrano.plugin.kag.tag.autoload = {
  ...
  pm: { title: '' },
  start: function (pm) {
    this.kag.menu.loadAutoSave(pm.title)
  },
}
tyrano.plugin.kag.tag.ignore = {
  ...
  start: function (pm) {
    if (this.kag.embScript(pm.exp)) {
      ...
      i > 1900 && this.kag.error('Ignore chưa được đóng')
...
tyrano.plugin.kag.tag.edit = {
  ...
  start: function (pm) {
    var j_text = $(
      "<input spellcheck='false' class='text_box form' name='" +
        pm.name +
        "' type='text' value='' />"
    )
...
tyrano.plugin.kag.tag.commit = {
  start: function () {
    ...
    this.kag.layer
      ...
      .find('.form')
      .each(function () {
        const escapedVal = $(this)
          .val()
          .replace(/\\/g, '\\\\')
          .replace(/'/g, "\\'")
        var str = $(this).attr('name') + " = '" + escapedVal + "'"
        console.log(str)
...
tyrano.plugin.kag.tag.check_web_patch = {
  ...
  checkPatch: function (obj, pm) {
    ...
    parseFloat(this.kag.variable.sf._patch_version) < parseFloat(obj.version)
      ? $.confirm(
          'Tìm thấy bản cập nhật mới. Ver:' +
            parseFloat(obj.version) +
            '「' +
            obj.message +
            '」<br />Bạn có muốn cập nhật không?',
          function () {
            alert(
              'Đang cập nhật. Sau khi hoàn tất, trò chơi sẽ tự động tắt.'
            )
