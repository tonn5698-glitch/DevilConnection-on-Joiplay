tyrano.plugin.kag.ftag = {
  ...
  showNextImg: function () {
    ...
    if (!this.kag.tmp.message_user_scrolled) {
      var _el = this.kag.getMessageInnerLayer()[0]
    ...
    !(function (jtext) {
      ...
      var append_span = j_span.children('span:last-child'),
        pchar = function (index) {
          if (!that.kag.tmp.message_user_scrolled) {
            var _el = j_msg_inner[0]
            if (_el) _el.scrollTop = _el.scrollHeight
          }
          var isOneByOne =
...
tyrano.plugin.kag.tag.label = {
  ...
  omitForRecord: str =>
    str
      ...
      .replace(/:/g, '')
      .replace(/\./g, ''),
  recordRecord: async function () {
    this.kag.variable.sf.record = Array.from(this.kag.tmp.record)
    this.kag.saveSystemVariable()
  },
  start: function (pm) {
    if ('true' == this.kag.config.autoRecordLabel) {
      // callスタックがある場合はプレフィックスを組み立てる
      const callStack = this.kag.stat.stack.call
      const callPrefix =
        callStack && callStack.length > 0
          ? callStack
              .map(c => `${this.omitForRecord(c.storage)}_${c.line}_`)
              .join('_')
          : ''
      var sf_tmp =
        'trail_' +
        callPrefix +
        this.omitForRecord(this.kag.stat.current_scenario)
      ;(sf_buff = this.kag.stat.buff_label_name),
        (sf_label = sf_tmp + '_' + pm.label_name)
      if (this.kag.tmp.record && '' != this.kag.stat.buff_label_name) {
        const buffCnt = this.kag.tmp.record.get(sf_buff) || 0
        this.kag.tmp.record.set(sf_buff, buffCnt + 1)
        this.recordRecord()
      }
      this.kag.tmp.record && this.kag.tmp.record.get(sf_label)
...
tyrano.plugin.kag.tag.cm = {
  start: function () {
    ...
    this.kag.tmp.message_user_scrolled = false
    this.kag.ftag.startTag('resetfont')
  },
}
    ...
    if ('others' == pm.name)
      ...
      if (null == cpm) {
        this.kag.error(
          'Nhân vật "' +
            pm.name +
            '" chưa được định nghĩa. Vui lòng dùng [chara_new] để định nghĩa.'
        )
        return
      }
      let _cpm = cpm.fuki
...
tyrano.plugin.kag.tag.image = {
  ...
  start: function (pm) {
    ...
    if ('base' != pm.layer) {
      ...
      strage_url =
        $.isHTTP(pm.storage) || pm.storage.match(/^[a-z]+:/)
          ? pm.storage
      ...
      pm.layer == 'fix' && img_obj.addClass('fixlayer')
      pm.layer == 'fix' && 'false' == pm.visible
        ? img_obj.hide()
        : img_obj.show()
      ...
      if ('' != pm.time) {
        ...
        img_obj
          ...
          .animate(
            ...
            parseInt(pm.time),
            function () {
              'true' == pm.wait && !pm.stop && that.kag.ftag.nextOrder()
            }
          )
        'true' != pm.wait && !pm.stop && that.kag.ftag.nextOrder()
      } else {
        'back' == pm.depth
          ? this.kag.layer.getLayer(pm.layer, pm.page).prepend(img_obj)
          : this.kag.layer.getLayer(pm.layer, pm.page).append(img_obj)
        !pm.stop && this.kag.ftag.nextOrder()
      }
    } else {
  ...
  start: function (pm) {
    ...
    if ('' != pm.time) {
      ...
      tobj.stop(!0, !0).animate({ opacity: 1 }, parseInt(pm.time), function () {
        pm.wait == 'true' && that.kag.ftag.nextOrder()
      })
      pm.wait != 'true' && this.kag.ftag.nextOrder()
    } else {
      target_layer.append(tobj)
      this.kag.ftag.nextOrder()
    }
...
tyrano.plugin.kag.tag.font = {
  ...
  start: function (pm) {
    ...
    pm.bgcolor && (this.kag.stat.font.bgcolor = $.convertColor(pm.bgcolor))
    ...
    !(function () {
      ...
      j_button.click(function (event) {
        ...
        if ('' == _pm.role && 'true' == _pm.fix) {
          ...
          if (null != stack_pm) {
            that.kag.log(
              'Khi còn call stack, nút fix sẽ không phản hồi'
            )
...
tyrano.plugin.kag.tag.glink = {
  ...
  start: function (pm) {
    ...
    target_layer =
      pm.layer == 'free'
        ? this.kag.layer.getFreeLayer().css('z-index', 999999)
        : this.kag.layer.getLayer(pm.layer)
    var j_button = $(
      "<div class='glink_button'><span>" + pm.text + '</span></div>'
    )
    'fix' == pm.layer && j_button.addClass('fixlayer')
    j_button.css('position', 'absolute')
    j_button.css('cursor', 'pointer')
    j_button.css('z-index', 99999999)
    ...
    if ('' != pm.graphic) {
      ...
      var img_url = pm.graphic.match(/^[a-z]+:/)
        ? pm.graphic
        : './data/image/' + pm.graphic
  ...
  setEvent: function (j_button, pm) {
    ...
    !(function () {
      ...
      var _pm = pm,
        preexp = that.kag.embScript(pm.preexp)
      const tapEvent = function (e) {
        '' != _pm.clickse &&
          that.kag.ftag.startTag('playse', { storage: _pm.clickse, stop: !0 })
        if (1 != that.kag.stat.is_strong_stop && pm.layer != 'fix') return !1
        !0
        '' != _pm.exp && that.kag.embScript(_pm.exp, preexp)
        that.kag.layer.showEventLayer()
      ...
      pm.mousedown ? j_button.mousedown(tapEvent) : j_button.click(tapEvent)
      j_button.hover(
        function () {
          if ('' != _pm.enterimg) {
          ...
          '' != _pm.enterse &&
            that.kag.ftag.startTag('playse', { storage: _pm.enterse, stop: !0 })
          '' != _pm.font_color_hover &&
            j_button.css('color', $.convertColor(_pm.font_color_hover))
        },
        function () {
          if ('' != _pm.enterimg) {
          ...
          '' != _pm.leavese &&
            that.kag.ftag.startTag('playse', { storage: _pm.leavese, stop: !0 })
          '' != _pm.font_color_hover &&
            j_button.css('color', $.convertColor(_pm.font_color))
        }
      )
    })()
  },
...
tyrano.plugin.kag.tag.clickable = {
  ...
  start: function (pm) {
    ...
    var j_button = $('<div />').addClass(pm.name)
    j_button.css('position', 'absolute')
    j_button.css('cursor', 'pointer')
    j_button.css('top', this.kag.stat.locate.y + 'px')
    j_button.css('left', this.kag.stat.locate.x + 'px')
  ...
  setEvent: function (j_button, pm) {
    ...
    !(function () {
      ...
      const handler = function () {
        ...
        pm.cm != 'false' && that.kag.ftag.startTag('cm', {})
        that.kag.layer.showEventLayer()
        that.kag.ftag.startTag('jump', _pm)
      }
      j_button.on(pm.mousedown ? 'mousedown' : 'click', handler)
    })()
  },
}
...
tyrano.plugin.kag.tag.bg = {
  ...
  start: function (pm) {
    ...
    pm.storage.match(/^[a-z]+:/) && (storage_url = pm.storage)
    this.kag.preload(storage_url, function () {
      if (0 == pm.time) {
        const bg = that.kag.layer.getLayer('base', 'fore')
        bg.css('background-image', 'url(' + storage_url + ')')
        if ('' != pm.position) {
          bg.css('background-size', 'cover')
          bg.css('background-position', pm.position)
        }
        return
      }
      const j_old_bg = that.kag.layer.getLayer('base', 'fore'),
        j_new_bg = j_old_bg.clone(!1)
      // j_old_bg以外のbaseレイヤーを削除
      $('.base_fore').each(function () {
        $(this).data('timestamp') != j_old_bg.data('timestamp') &&
          $(this).remove()
      })
      ...
      j_new_bg.data('timestamp', new Date().getTime())
...
tyrano.plugin.kag.tag.layermode = {
  ...
  start: function (pm) {
    ...
    var that = this,
      blend_layer = null
    blend_layer = $(
      "<div class='layer_blend_mode blendlayer' style='display:none;position:absolute;width:100%;height:100%'></div>"
    ).css('z-index', pm.zindex)
    ...
    if (!$.supportsBlendMode && !pm.opacity) {
      blend_layer.css('opacity', '0.4')
    }
    this.kag.layer.getLayer('fix').append(blend_layer)
    '' != pm.graphic
      ? this.kag.preload(storage_url, function () {
...
tyrano.plugin.kag.tag.layermode_movie = {
  ...
  start: function (pm) {
    ...
    video.style.backgroundColor = $.supportsBlendMode ? 'black' : 'transparent'
    ...
    video.addEventListener('error', function () {
      blend_layer.remove()
      if ('true' == pm.wait && 'true' != pm.stop && that.kag.ftag) {
        that.kag.ftag.nextOrder()
      }
    })
    video.load()
    ...
    setTimeout(function () {
      if (blend_layer.parent() && blend_layer.parent().length) {
        blend_layer.remove()
      }
    }, 5000)
  },
