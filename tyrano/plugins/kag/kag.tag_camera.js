tyrano.plugin.kag.tag.camera = {
  ...
  start: function (pm) {
    const layers =
      pm.layer == 'layer_camera'
        ? [...$('.layer_camera').not('[style*="display: none"]')].map(
            e => e.className.match(/(base|[0-9])_(fore|back)/)[1]
          )
        : [pm.layer]
    ...
    layers.forEach(layer => {
      void 0 === this.kag.stat.current_camera[layer] &&
        (this.kag.stat.current_camera[layer] = {
          x: '0',
          y: '0',
          scale: '1',
          rotate: '0',
        })
      var to_camera = $.extend(!0, {}, this.kag.stat.current_camera[layer])
      ...
      var a3d_define = {
        frames: {
          '0%': { trans: this.kag.stat.current_camera[layer] },
          '100%': { trans: to_camera },
        },
        config: { duration: duration, state: 'running', easing: pm.ease_type },
        complete: function () {
          // complete前のカメラ設定で上書きしないように、直近のカメラ設定を参照する
          const c = that.kag.stat.current_camera[layer]
          if (that.isCameraReset(to_camera) && that.isCameraReset(c)) {
            $(`.${layer}_fore`).css({
              '-animation-name': '',
              '-animation-duration': '',
            ...
            delete that.kag.stat.current_camera[layer]
          }
          layerCount--
          if ('true' == pm.wait && 0 == flag_complete && layerCount == 0) {
            flag_complete = !0
            setTimeout(function () {
      ...
      this.kag.stat.current_camera[layer] = to_camera
      $('.' + layer + '_fore').css('-webkit-transform-origin', 'center center')
      $('.' + layer + '_fore').a3d(a3d_define)
    })
  ...
  isCameraReset: c => {
    if (!c) return false
    if (!c.x && !c.y && !c.scale && !c.rotate) return false
    return c.x == '0px' && c.y == '0px' && c.scale == '1' && c.rotate == '0deg'
  },
...
tyrano.plugin.kag.tag.mask = {
  ...
  start: function (pm) {
    ...
    that.kag.layer.getLayer('fix').append(j_div)
    j_div
