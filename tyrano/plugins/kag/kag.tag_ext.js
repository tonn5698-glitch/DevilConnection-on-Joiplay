...
tyrano.plugin.kag.tag.movie = {
  ...
  playVideo: function (pm) {
    var that = this,
      url = pm.storage.includes(':')
        ? pm.storage
        : './data/video/' + pm.storage,
    ...
    if (sfMovieVolume) volume *= parseFloat(parseInt(sfMovieVolume) / 100)
    video.volume = volume
    $(video).attr('volume', pm.volume).addClass(pm.name)
    ...
    var j_video = $(video)
    parseInt(pm.time) && j_video.css('opacity', 0)
...
tyrano.plugin.kag.tag.stop_bgmovie = {
  vital: [],
  pm: { time: '300', wait: 'true', stop: false },
  start: function (pm) {
    var that = this
    that.kag.tmp.video_playing = !1
...
tyrano.plugin.kag.tag.kanim = {
  vital: ['keyframe'],
  pm: { name: '', layer: '', keyframe: '', stop: false },
  start: function (pm) {
    var that = this,
    ...
    if ('' != pm.name) {
      this.kag.stat.playingKeyFrameName ||
        (this.kag.stat.playingKeyFrameName = {})
      this.kag.stat.playingKeyFrameName[pm.name] = pm.keyframe
      anim.complete = function () {
        that.kag.popAnimStack()
        delete that.kag.stat.playingKeyFrameName[pm.name]
      }
      delete pm.name
      delete pm.keyframe
      ...
      var layer_name = pm.layer + '_fore'
      'free' == pm.layer && (layer_name = 'layer_free')
      this.kag.stat.playingKeyFrameLayer ||
        (this.kag.stat.playingKeyFrameLayer = {})
      this.kag.stat.playingKeyFrameLayer[layer_name] = pm.keyframe
      anim.complete = function () {
        that.kag.popAnimStack()
        delete this.kag.stat.playingKeyFrameLayer[layer_name]
      }
      delete pm.name
      delete pm.keyframe
      delete pm.layer
...
tyrano.plugin.kag.tag.stop_kanim = {
  ...
  start: function (pm) {
    if ('' != pm.name) {
      ...
      this.kag.stat.playingKeyFrameName &&
        delete this.kag.stat.playingKeyFrameName[pm.name]
    } else if ('' != pm.layer) {
      var layer_name = pm.layer + '_fore'
      'free' == pm.layer && (layer_name = 'layer_free')
      $('.' + layer_name).css({
      ...
      if (!this.kag.stat.charas[pm.name].map_face[pm.face]) {
        this.kag.error(
          'Nhân vật "' +
            ...
            '" hoặc face "' +
            pm.face +
            '" chưa được định nghĩa. Vui lòng kiểm tra lại.'
        )
        return
      }
...
'Nhân vật 「' +
        ...
        for (key in chara_layer) {
          ...
          'allow_storage' == current_part_id &&
            (chara_obj = { storage: chara_part.allow_storage, visible: 'true' })
...
tyrano.plugin.kag.tag.chara_hide_all = {
  ...
  start: function (pm) {
    ...
    for (let key in charas) {
      ...
      this.kag.stat.playingKeyFrameName &&
        delete this.kag.stat.playingKeyFrameName[key]
    }
    if (img_obj.get(0)) {
      img_obj.stop(!0, !0).animate(
        { opacity: 'hide' },
    ...
    if (null != cpm)
      if (cpm._layer) {
        ...
        this.kag.error(
          'Nhân vật "' +
            pm.name +
            '" chưa có phần hình ảnh chênh lệch. Vui lòng dùng [chara_layer] để định nghĩa.'
        )
    else
      this.kag.error(
'Nhân vật "' +
...
tyrano.plugin.kag.tag.filter = {
  ...
  start: function (pm) {
    ...
    pm.time !== 0
      ? j_obj.css('transition', `filter ${pm.time / 1000}s linear`)
      : j_obj.css('transition', '')
    j_obj.css({
...
tyrano.plugin.kag.tag.free_filter = {
  vital: [],
  pm: { layer: '', page: 'fore', name: '', time: 0 },
  start: function (pm) {
    let j_obj
    j_obj =
      '' == pm.layer
        ? $('.tyrano_filter_effect,.filter_removing')
        : this.kag.layer.getLayer(pm.layer, pm.page)
    '' != pm.name && (j_obj = j_obj.find('.' + pm.name))
    pm.time !== 0
      ? j_obj.css('transition', `filter ${pm.time / 1000}s linear`)
    ...
    j_obj.addClass('filter_removing').removeClass('tyrano_filter_effect')
    j_obj.on('transitionend', function () {
      j_obj.removeClass('filter_removing')
    })
