var throttle = (fn, limit) => {
  ...
  return () => {
    ...
    lastFunctionTimerId = setTimeout(function () {
      fn()
      lastExecute = Date.now()
      // 指定時間以上経過している場合は即実行、そうでなければ残り時間経過後に実行
...
tyrano.plugin.kag.key_mouse = {
  ...
  esc: function () {
    if (
      this.kag.stat.current_scenario == 'title_screen.ks' &&
      this.kag.tmp.escInTitle
    ) {
      this.kag.ftag.startTag('jump', { target: '*close_confirm' })
    } else {
      this._role('menu')
    }
  },
  _role: function (role) {
    ...
    switch (role) {
      ...
      case 'menu':
        ...
        break
      case 'skip':
        $('.skip_button.event-setting-element:visible').length > 0 &&
          this.kag.layer.getMenuLayer().css('display') == 'none' &&
          this.kag.ftag.startTag('skipstart', {})
        break
      case 'backlog':
        $('.log_button.event-setting-element:visible').length > 0 &&
          this.kag.layer.getMenuLayer().css('display') == 'none' &&
          1 != this.kag.stat.is_adding_text &&
  ...
  canClick: function () {
    return (
      ...
      'none' == $('.layer_menu').css('display') &&
      $('.guard_click,.flash').length == 0
    )
  },
  clearSkip: function () {
    // See: data/others/plugin/skipstart_se/main.js
