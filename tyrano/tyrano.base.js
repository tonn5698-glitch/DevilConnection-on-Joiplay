  ...
  _fitBaseSize: function (width, height) {
    ...
    if (screen_ratio == 'fix') {
      ...
      setTimeout(function () {
        ...
        if (!$('#scale_container')[0]) {
          $('.tyrano_base').wrapInner('<div id="scale_container">')
        }
        //中央寄せなら、画面サイズ分を引く。
        if (
          ...
          $('#scale_container').css({
            width: `${that.tyrano.kag.config.scWidth}`,
            height: `${that.tyrano.kag.config.scHeight}`,
            'transform-origin': '0 0',
            position: 'relative',
          })
          $('.tyrano_base').css({
            margin: 0,
            width: `${Math.floor(that.tyrano.kag.config.scWidth * scale_f)}px`,
            height: `${Math.floor(
        ...
        if (
          ...
          var base_height = Math.round(
            parseInt($('#tyrano_base').css('height'))
          )
