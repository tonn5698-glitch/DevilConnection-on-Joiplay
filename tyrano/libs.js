;(function ($) {
  ...
  $.setStorageWeb = function (key, val) {
    ...
    try {
      ...
      console.error('Dữ liệu save. Không thể sử dụng localstorage.')
      return
    }
  }
  ...
  $.confirmSaveClear = function () {
    if (
      confirm(
        'Dữ liệu save có thể bị hỏng. Bạn có muốn xóa dữ liệu save không?'
      )
    ) {
      alert('Đã xóa')
      localStorage.clear()
  ...
  $.clearStorage = function (type, key = '') {
    if (type == 'file') {
      ...
      const storagePath = $.getExePath() + '/storage' + file
      fs.existsSync(storagePath) && fs.removeSync(storagePath)
    } else {
  ...
  $.getStorageCompress = function (key) {
    try {
      ...
      console.log(e)
      alert(
        'Môi trường này không thể sử dụng chức năng save. Điều này có thể xảy ra khi chạy trên máy cục bộ.'
      )
      $.confirmSaveClear()
    }
  ...
  $.setStorageFile = function (key, val) {
    ...
    if (!fs.existsSync(out_path + '/storage/'))
      fs.mkdirSync(out_path + '/storage')

    fs.writeFileSync(out_path + '/storage/' + key + '.sav', escape(val))
  }
  ...
  $.getStorageFile = function (key) {
    try {
      ...
      if (fs.existsSync(out_path + '/storage/' + key + '.sav')) {
        var str = fs.readFileSync(out_path + '/storage/' + key + '.sav')
        gv = unescape(str)
      } else {
  ...
  $.saveFile = async function (dataUrl) {
    ...
    const path = dialog
      ? dialog.showSaveDialogSync({
          title: $.lang('save_photo'),
          filters: [{ name: 'PNG', extensions: 'png' }],
          defaultPath: 'photo.png',
        })
      : `C:/Users/${require('os').userInfo().username}/Pictures/photo.png`
    if (!path) return
  ...
  /*
    ...
    alert("Môi trường này không thể sử dụng chức năng save. Điều này có thể xảy ra khi chạy trên máy cục bộ.");
    }
  ...
  $.alert = function (str, cb, buttonText = 'OK') {
    $('.remodal_title').html(str)

    $('.remodal').find('.remodal-cancel').hide()
    $('.remodal').find('.remodal-confirm').text(buttonText).show()
  ...
  $.confirm = function (
    ...
    cb_cancel,
    text_ok = $.lang('ok'),
    text_cancel = $.lang('cancel')
  ) {
    if ($('.remodal-wrapper').css('display') !== 'none') {
      setTimeout(
        () => $.confirm(str, cb_ok, cb_cancel, text_ok, text_cancel),
        10
      )
      return
    }
