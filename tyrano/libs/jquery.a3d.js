...
;(function ($) {
  ...
  $.fn.a3d = function (arg) {
    ...
    return this.each(function (i) {
      ...
      var id_suffix = arg.id_suffix || ''

      var point = { x: 0, y: 0, z: 0 }

      var anim_id = 'id_' + Math.floor(Math.random() * 1000000000)
      id_suffix && (anim_id += `_${id_suffix}`)
      ...
      for (percentage in keyframe) {
        ...
        frames = convertTrans(
          trimAll(keyframe[percentage].trans),
          point,
          base_point
        )
      ...
      lastSheet.insertRule(
        '@-' + vendor + '-keyframes ' + anim_id + ' {' + master_text + '} ',
      ...
      if (true) {
        ...
        obj
          .css(css_param)
          .one('' + vendor + 'AnimationEnd animationend', function (e) {
        ...
        obj
          .css(css_param)
          .bind(
            '' + vendor + 'AnimationIteration animationiteration',
            function (e) {
              if (obj.get(0) !== e.target) {
                return false
              }
        ...
        obj
          .css(css_param)
          .one('' + vendor + 'AnimationStart animationstart', function (e) {
            if (obj.get(0) !== e.target) {
              return false
            }
  ...
  function trimAll(array_obj) {
    ...
    for (key in array_obj) {
      ...
      result[key.replace(/^\s+|\s+$/g, '')] = array_obj[key].replace(
        /^\s+|\s+$/g,
        ''
      )
    }
