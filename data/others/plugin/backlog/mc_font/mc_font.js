...
;(function () {
  ...
  const font_style = function (current) {
    ...
    if (font.edge) {
      ...
      style += 'text-shadow:' + '2px 2px 2px ' + font.shadow + ';'
  ...
  tag.mc_font = {
    ...
    start: function (pm) {
      ...
      if (pm.color)
        TYRANO.kag.stat.memocho.log.font.color = $.convertColor(pm.color)
      if (pm.bold)
        TYRANO.kag.stat.memocho.log.font.bold = $.convertBold(pm.bold)
      if (pm.face) TYRANO.kag.stat.memocho.log.font.face = pm.face
      if (pm.italic)
        TYRANO.kag.stat.memocho.log.font.italic = $.convertItalic(pm.italic)
      if (pm.edge)
        TYRANO.kag.stat.memocho.log.font.edge = $.convertColor(pm.edge)
      if (pm.shadow)
        TYRANO.kag.stat.memocho.log.font.shadow = $.convertColor(pm.shadow)
      if (pm.effect) {
        if (pm.effect == 'none') TYRANO.kag.stat.memocho.log.font.effect = ''
        else TYRANO.kag.stat.memocho.log.font.effect = pm.effect
      }
      if (pm.effect_speed)
        TYRANO.kag.stat.memocho.log.font.effect_speed = pm.effect_speed
  ...
  tyrano.plugin.kag.pushBackLog = function (str, type) {
    ...
    if (TYRANO.kag.stat.memocho.log.font.name) {
      ...
      if (isLayer.find('p').find('.current_span').length == 0) {
        ...
        $.setName(
          ...
          TYRANO.kag.stat.memocho.log.font.name
        )
      }
    }
    /////end////////////////////////////////////////////////////
    ...
    if (type == 'join') {
      ...
      //add
      const backlogCharaNamePrefix = /^<b class=['"]backlog_chara_name/
      const meImgPrefix = /^<img src=".\/data\/image\/me.png"/
      const currentLog = this.variable.tf['system']['backlog'][index]

      if (str.startsWith('</')) {
        // 閉じタグを入れようとしてたらjoinと同様に扱う
        this.variable.tf['system']['backlog'][index] = currentLog + str
      } else if (
        this.variable.tf['system']['backlog'].length == 0 ||
        currentLog.match(backlogCharaNamePrefix) ||
        currentLog.match(meImgPrefix)
      ) {
        // バックログの先頭が所定のフォーマットであれば次のログへ
        // バックログが空でもそのまま追加する
        this.variable.tf['system']['backlog'].push(str)
      } else if (
        str.match(backlogCharaNamePrefix) ||
        currentLog.match(meImgPrefix)
      ) {
        // 追加しようとしているログが所定のフォーマットであれば次のログに行かず先頭に追加
        const row = [
          str.substring(0, str.indexOf('<span')),
          str.substring(str.indexOf('<span')),
        ]
        this.variable.tf['system']['backlog'][index] =
          row[0] + currentLog + row[1]
      } else {
        // 所定のフォーマットでなければ次の行に行かずそのまま追加する
        this.variable.tf['system']['backlog'][index] = currentLog + str
      }
    }
