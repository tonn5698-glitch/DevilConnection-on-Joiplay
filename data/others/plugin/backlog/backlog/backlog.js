...
;(function () {
  ...
  TYRANO.kag.tmp.memocho.log.name_color =
    TYRANO.kag.stat.mp.name_color || 'false'
  TYRANO.kag.tmp.memocho.log.name_none = TYRANO.kag.stat.mp.name_none || 'false'
  TYRANO.kag.tmp.memocho.log.name_repeat =
    TYRANO.kag.stat.mp.name_repeat || 'false'
  TYRANO.kag.tmp.memocho.log.text_center =
    TYRANO.kag.stat.mp.text_center || 'false'
  TYRANO.kag.tmp.memocho.log.vertical =
    TYRANO.kag.stat.mp.vertical || TYRANO.kag.config.vertical
  TYRANO.kag.tmp.memocho.log.text_count = 0

  // [l]でaddにならないように。
  tyrano.plugin.kag.tag.l.log_join = 'true'
      ...
      if (this.master_tag[tag.name]) {
        ...
        if (this.master_tag[tag.name].log_join == 'true') {
          ...
          if (tag.name == 'text') {
            ...
            if (TYRANO.kag.tmp.memocho.log.text_count == 0)
        ...
        if (err_str != '') {
          ...
          this.master_tag[tag.name].start(
            $.extend(true, $.cloneObject(this.master_tag[tag.name].pm), tag.pm)
          )
        }
  ...
  tyrano.plugin.kag.menu.displayLog = function () {
    ...
    this.kag.html(
      ...
      function (html_str) {
        var j_menu = $(html_str)
        that.kag.variable.sf.killed == 1 &&
          j_menu
            .find('.img_bg_base')
            .attr(
              'src',
              'tyrano/images/system/builder/bg_base_backlog_backlog_kill.png'
            )
        ...
        layer_menu.find('.menu_close').click(function () {
          ...
          const bgmOptions = {
            config: parseInt(that.kag.config.defaultBgmVolume) / 100,
            tag: 0.4,
            time: 300,
          }
          that.kag.stat.current_bgm
            ? changeHowlVolume(that.kag.tmp.map_bgm['0'], bgmOptions)
            : that.kag.ftag.startTag('lbgmvol', {
                vol: 40,
                time: 300,
                stop: 'true',
              })
          const seOptions = {
            config: parseInt(that.kag.config.defaultSeVolume) / 100,
            tag: 1,
            time: 300,
          }
          that.kag.stat.current_se &&
            that.kag.stat.current_se['5'] &&
            that.kag.tmp.map_se['5'].playing() &&
            changeHowlVolume(that.kag.tmp.map_se['5'], seOptions)
          that.kag.ftag.startTag('lsestop', {
            time: '300',
            stop: 'true',
            buf: '2',
          })
        ...
        const mindVoices = that.kag.stat.mindVoices || []
        mindVoices.forEach(v => {
          const { name, text, color, face } = v
          const mindVoiceLog = `
            <b class="backlog_chara_name mind_voice ${name}">${name}</b>
            <p class="log_text">
              <span class="backlog_text mind_voice ${name}" style="color: ${color.replace(
            '0x',
            '#'
          )}; font-family: ${face || 'inherit'}">${text}</span>
            </p>
          `
          array_log = array_log.concat(mindVoiceLog)
        })
            ...
            for (let i = 0; i < logHTML.length; i++) {
              if (logHTML[i]['nodeName'] != '#comment') {
                ...
                if (logHTML[i]['className'].indexOf('glink') > -1) {
                  charaEName = 'glink'
                  new_log =
                    new_log.replace('<span', '<p class="log_text"><span') +
                    '</p>'
                }
                //キャラ名があるか
                  ...
                  //日本語名なら英数名に
                  charaEName =
                    TYRANO.kag.stat.jcharas[classList[1]] || classList[1]
                  //名前の色
                  if (
                    log.name_color == 'true' &&
                    TYRANO.kag.stat.charas[charaEName]
                  ) {
                    charaColor = TYRANO.kag.stat.charas[charaEName].color || ''
                    charaColor =
                      ' style="color:' + $.convertColor(charaColor) + '"'
                    new_log = new_log.replace('<b', '<b' + charaColor)
                  }
                  //テキストを p で囲う
                  new_log =
                    new_log.replace('</b>：', '</b><p class="log_text">') +
                    '</p>'
                }
                //メッセージか
                  ...
                  new_log =
                    '<b class="backlog_chara_name"></b><div class="log_text">' +
                    new_log +
                    '</div>'
                }
                //pushLog
        ...
        if (log.name_none == 'true')
          layer_menu.find('.log_body').addClass('name_none')
        if (log.name_repeat == 'true')
          layer_menu.find('.log_body').addClass('name_repeat')
        else if (log.text_center == 'true')
          layer_menu.find('.log_body').addClass('center')
        // END /////////////////////////////////////////////////

        const { censorship } = that.kag.variable.sf
        layer_menu
          .find('.log_body')
          ...
          .css('background-color', function () {
            return censorship ? $(this).css('color') : ''
          })
          ...
          if ($.userenv() != 'pc') {
            ...
            layer_menu
              ...
              .click(function () {
                ...
                layer_menu
                  .find('.log_body')
                  .animate({ scrollLeft: pos }, { queue: false })
              })
            layer_menu
        ...
        $('.button_menu').hide()
        bindHover('.menu_close', '.menu_close', 'modoru.png', 'modoru2.png')
        const bgmOptions = {
          config: parseInt(that.kag.config.defaultBgmVolume) / 100,
          tag: 0.04,
          time: 300,
...
// 心の声
// mode=setで上書き、mode=appendで追加
TYRANO.kag.ftag.master_tag.mind_voice = {
  vital: ['name', 'text'],
  pm: {
    name: '',
    text: '',
    color: '0xf566ba',
    face: 'KaiseiDecol-Bold',
    mode: 'set',
  },
  start: function (pm) {
    TYRANO.kag.stat.mindVoices =
      pm.mode == 'append' ? [...TYRANO.kag.stat.mindVoices, pm] : [pm]
    TYRANO.kag.ftag.nextOrder()
  },
}

TYRANO.kag.ftag.master_tag.reset_mind_voice = {
  start: function () {
    TYRANO.kag.stat.mindVoices = []
    TYRANO.kag.ftag.nextOrder()
  },
...
// fontタグによる色変更をバックログに適用する
;(function () {
  const fontTag = TYRANO.kag.tag.font
  const fontStart = fontTag.start

  fontTag.start = function (pm) {
    const currentStyle = TYRANO.kag.stat.font
    const newStyle = pm

    this.kag.tmp.backlog_color && this.kag.pushBackLog(`</span>`, 'join')
    const cStyle = newStyle.color
      ? `color: ${$.convertColor(newStyle.color)};`
      : currentStyle.color
      ? `color: ${currentStyle.color};`
      : ''
    const bcStyle = newStyle.bgcolor
      ? `background-color: ${$.convertColor(newStyle.bgcolor)};`
      : currentStyle.bgcolor
      ? `background-color: ${currentStyle.bgcolor};`
      : ''

    if (cStyle || bcStyle) {
      this.kag.pushBackLog(`<span style="${cStyle}${bcStyle}">`, 'join')
      this.kag.tmp.backlog_color = true
    }

    fontStart.call(this, pm)
  }

  const resetfontTag = TYRANO.kag.tag.resetfont
  const resetfontStart = resetfontTag.start

  resetfontTag.start = function (pm) {
    if (this.kag.tmp.backlog_color) {
      this.kag.pushBackLog(`</span>`, 'join')
      this.kag.tmp.backlog_color = false
    }
    resetfontStart.call(this, pm)
  }
})()
