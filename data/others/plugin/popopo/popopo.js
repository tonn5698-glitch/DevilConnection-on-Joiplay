;(function ($, TYRANO) {
  ...
  newtag.popopo = {
    ...
    start: function (pm) {
      ...
      console.log(this.kag.variable.sf._config_ch_speed)
      if (this.kag.variable.sf._config_ch_speed > popopo.interval) {
        popopo.mode = 'everyone'
      } else {
        popopo.mode = 'interval'
      }
      ...
      if (typeof pm.noplaychars === 'string')
        popopo.noplaychars = pm.noplaychars
  ...
  TYRANO.kag.popopo.audioContext.createGain =
    TYRANO.kag.popopo.audioContext.createGain ||
    TYRANO.kag.popopo.audioContext.createGainNode
  ...
  TYRANO.kag.popopo.file = {
    ...
    interval: {
      playse: throttle(() => {
        TYRANO.kag.ftag.startTag('playse', {
          volume: TYRANO.kag.stat.popopo.volume,
          buf: TYRANO.kag.stat.popopo.buf,
          storage: TYRANO.kag.stat.popopo.storage,
          stop: true,
        })
      }, TYRANO.kag.stat.popopo.interval || 100),
      start: function (message_str, ch_speed) {
        var pm = TYRANO.kag.stat.popopo
  ...
  TYRANO.kag.popopo.createNode = function (pm) {
    if (pm.type === 'noise') {
      ...
      node.type =
        typeof node.type === 'string' ? pm.type : TYPE_TO_NUMBER[pm.type]
  ...
  TYRANO.kag.popopo.noise = {
    ...
    createNoise: function (pm) {
      ...
      var sampleRate = Math.min(
        Math.max(3000, parseInt(pm.samplerate) || 44000),
        192000
      )
      var noiseBuffer
  ...
  TYRANO.kag.popopo.wave = {
    ...
    everyone: {
      ...
      play: function (ch) {
        ...
        TYRANO.kag.popopo.gainNode.gain.setTargetAtTime(
          0,
          t0 + pm.time,
          pm.tailtime
        )
  ...
  tyrano.plugin.kag.tag.configdelay.start = function (pm) {
    if (pm.speed != '') {
      ...
      const popopo = this.kag.stat.popopo
      if (pm.speed > popopo.interval) {
        popopo.mode = 'everyone'
      } else {
        popopo.mode = 'interval'
      }
  ...
  tyrano.plugin.kag.tag.delay.start = function (pm) {
    if ('' != pm.speed) {
  ...
  tyrano.plugin.kag.tag.resetdelay.start = function () {
    this.kag.stat.ch_speed = ''
    const popopo = this.kag.stat.popopo
    if (this.kag.variable.sf._config_ch_speed > popopo.interval) {
      popopo.mode = 'everyone'
  ...
  //文字揺れを開始する
  tyrano.plugin.kag.ftag.master_tag.quake_text = {
    pm: {},
    kag: TYRANO.kag,
    start: function () {
      this.kag.stat.quake_text = true
      this.kag.ftag.nextOrder()
    },
  }

  //文字揺れを終了する
  tyrano.plugin.kag.ftag.master_tag.free_quake_text = {
    pm: {},
    kag: TYRANO.kag,
    start: function () {
      this.kag.stat.quake_text = false
      this.kag.ftag.nextOrder()
    },
  }
  ...
  TYRANO.kag.tag.text.showMessage = function (message_str, pm) {
    ...
    ;(function (jtext) {
      ...
      if (jtext.find('p').find('.current_span').length != 0) {
        ...
        //テキストボックスの中身が空の場合文字揺れ用のCSSを削除
        const lastSheet = document.styleSheets[document.styleSheets.length - 1]
        ;[...lastSheet.cssRules]
          .reduce(
            (acc, rule, index) =>
              rule.name && rule.name.includes('quake_text')
                ? acc.concat(index)
                : acc,
            []
          )
          .sort((a, b) => b - a)
          .forEach(i => lastSheet.deleteRule(i))
      }
      var index = 0
      //jtext.html("");
      ...
      j_span.css({
        ...
        'background-color': that.kag.stat.font.bgcolor,
      })
      ...
      var pchar = function (pchar) {
          ...
          const a3d_define = {
            frames: [...Array(5)]
              .map((_, i) => {
                ...
                return {
                  [`${i * 20}%`]: { trans: { x, y } },
                  [`${i * 20 + 19.99}%`]: { trans: { x, y } },
                }
              })
              .reduce((acc, cur) => ({ ...acc, ...cur }), {}),
            config: { duration: '0.75s', count: 'infinite' },
            complete: function () {},
            id_suffix: 'quake_text',
          }
          const quakec = $(`<span>${c}</span>`).css('display', 'inline-block')
          quakec.a3d(a3d_define)
          qc = quakec.prop('outerHTML')
          current_str += qc
        } else {
          current_str += c
        }
        ...
        if (
          ...
          that.kag.stat.quake_text
            ? that.kag.appendCharToMessage(jtext, qc)
